...
export default function Chat() {
    ...
    const [thisChat, setThisChat] = useState(Chats);

    const [messageList, setMessageList] = useState([]);

    useEffect(() => {
        setMessageList(() => thisChat[chatId].messages)
    }, [chatId, thisChat]);
    ...
    useEffect(() => {
        ...
        if (messageList.length === 0) {
            ...
            setThisChat((prevThisChat) => {
                prevThisChat[chatId].messages = messageList;
                return prevThisChat;
            });
        }
            ...
            <section className="content message">
                ...
                <div className="message-list">
                    <Typography className="chat-header">
                        Chat with <span>{thisChat[chatId].name}</span>
                    </Typography>
                    <Divider />
